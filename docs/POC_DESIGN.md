...
- 回帰の番人として、初回計測値より十分低い下限（例：初回値−0.15）をintegration testに置き、初回値と下限を`POC_RESULTS.md`に記録する。**下限をテストが通るように後から下げない**。下回ったら原因を調べて記録する。
  → **この「−0.15」はフェーズ2.5で不十分と判明した**（無情報予測器が通過する。§12.4 Q-H1）。
  現行の規則は「**質問を読まない予測器のスコアと初回計測値の間**に置く」であり、基準線はfixtureから計算する。
...
> **上位文書との差分（解決済み）**: 仕様書 §7.7 は当初 PoC の backend を `a0-nli-zeroshot-v1` と名付けていた。
> escape 規則は「テキスト → token ID」という backend の契約そのものであり、同じ名前で別の系列を作ることは §5.7 の趣旨に反するため、実装を v2 に上げた。
> 仕様書 v0.3.0 の §7.7 を「`a0-nli-zeroshot`系列、現行は`v2`」へ改訂し、この差分は解消した（2026-09-21）。
